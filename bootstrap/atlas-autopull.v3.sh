...
#   v3 meta-agent:
#     P. PY3-ROBUST PARSER: jget prefers /opt/atlas/venv py3, then python3, then python;
#        real parse errors go to the LOG, never /dev/null.
#     S. RELIABLE STATUS-BACK: report() writes status/<node>/seq-<N>-<status>.json AND
#        a status/<node>/last-seen.json heartbeat to the repo via the GitHub Contents API,
#        with create+update (sha), JSON-safe fields, and HTTP-code logging (no silent fail).
#        Pushes on EVERY outcome incl. failures and early die().
#     Q. DEPLOY QUEUE: each cycle drains manifests/seq-<last+1>.json upward, in order,
#        one at a time, stopping on first failure (retried next cycle). Falls back to the
#        legacy single manifest.json if no numbered file exists.
#     N. MULTI-NODE: NODE_ID namespaces state + status; a step may carry "nodes":[...] to
#        target specific nodes (others skip it). Guardrail still scans ALL steps.
#     R. SELF-HEAL: transient fetch/network/source errors just wait for the next cycle;
#        a failed apply does NOT advance last_seq, so it retries; nothing wedges.
#
#   Self-test: --selftest (guardrail + rollback)  |  --rollback-selftest  |  --status-selftest
#
# Exit (per-cycle is best-effort; the timer reruns every 2 min):
...
# ===================================================================== #
#  SELF-TESTS (mutate only temp dirs / a mock endpoint)
# ===================================================================== #
...
if [ "${1:-}" = "--selftest" ]; then r=0; selftest_guardrail||r=1; rollback_selftest||r=1
...
# ---------------- status-back (GitHub Contents API) ----------------
json_str(){ "$PYBIN" -c 'import json,sys;print(json.dumps(sys.argv[1]))' "$1"; }
build_status_json(){ # <status> <detail>
  ST="$1" DETAIL="$2" NODE="$NODE_ID" SEQv="${SEQ:-0}" CF="$STATE_DIR/last_counts.json" VER="v3" "$PYBIN" - <<'PY'
import os,json,time
...
report(){ # <status> <detail...>
  ...
  gh_put "status/${NODE_ID}/seq-${SEQ:-0}-${st}.json" "$b64" "status ${NODE_ID} seq=${SEQ:-0} ${st}" || true
}
heartbeat(){
  local last; last="$(cat "$STATE_DIR/last_seq" 2>/dev/null || echo 0)"
  SEQ="$last"
  local body; body="$(build_status_json heartbeat "alive last_seq=$last")"
  local b64; b64="$(printf '%s' "$body" | base64 | tr -d '\n')"
...
log "parser=$PYBIN node=$NODE_ID brain=$BRAIN_URL api=$STATUS_API_BASE"
...
process_current(){
  ...
  local TS SNAP; TS="$(date -u +%Y%m%dT%H%M%SZ)"; SNAP="$RESTORE_ROOT/$SEQ-$TS"; mkdir -p "$SNAP" || { report failed "cannot make restore point"; return 50; }
  ...
  log "=== APPLY COMPLETE seq=$SEQ (healthy) ==="; report ok "applied seq=$SEQ steps=$NSTEPS healthy"; return 0
}

# ===================================================================== #
#  MAIN: heartbeat + drain the queue (manifests/seq-N.json), fall back to legacy
# ===================================================================== #
...
while [ "$n" -lt "$MAX_DRAIN" ]; do
  ...
  if curl -fsS --max-time 20 "$BRAIN_URL/manifests/seq-$NEXT.json" -o "$WORK/manifest.json" 2>/dev/null \
     && curl -fsS --max-time 20 "$BRAIN_URL/manifests/seq-$NEXT.json.sig" -o "$WORK/manifest.sig" 2>/dev/null; then
    ...
    log "queue: no seq-$NEXT, trying legacy manifest.json"
  else
    [ "$applied" -gt 0 ] && log "queue drained ($applied applied this cycle)" || log "nothing new (queue empty / network) -> wait next cycle"
    break
  fi
  ...
  case "$rc" in
    ...
    2|3) break;;
    *) log "stop drain (rc=$rc); will retry next cycle"; break;;
  esac
done
heartbeat
log "pull end node=$NODE_ID applied=$applied"
exit 0
